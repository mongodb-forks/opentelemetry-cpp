...
public:
  ...
  public:
    ...
    nostd::string_view GetKey() const noexcept { return key_; }

    // Gets the value associated with this entry.
    nostd::string_view GetValue() const noexcept { return value_; }

    // Sets the value for this entry. This overrides the previous value.
    void SetValue(nostd::string_view value) noexcept { value_ = value; }

  private:
    nostd::string_view key_;
    nostd::string_view value_;
  };
...
public:
  ...
  KeyValueProperties(size_t size) noexcept
      : num_entries_(0), max_num_entries_(size), entries_(size)
  {}

  // Create Empty Key-Value list
  KeyValueProperties() noexcept : num_entries_(0), max_num_entries_(0) {}
